...
2. It has to be consistent, i.e. for every node $n$ and for every neighbor $p$ of $n$, the following formula must apply: $ h(n) <= w(n, p) + h(p), $ where $w(n, p)$ is the weight of the edge between nodes $n$ and $p$. 
...
The layouts of the queries are shown in @astar_init. Both variants use the same column names. Every row in `astar` represents a node in the graph, which consists of 
...
To follow along, @astar_example illustrates each step for our running example in the first five iterations. 
...
The classic variant of A\* can be viewed as an extension of using-key. We follow the same approach; however, because we can only access values which have been calculated in the previous iteration, we have to carry all values manually. This vastly increases the size of the union table.
...
In @astar_example, the state of the recurring table after each iteration is shown in the last column. We can see it growing slowly. To compare its size to the union table of classic, in @astar_table_size_comparison, we list the size of the union- and recurring table at each iteration. Notice how the union table grows much faster.
