...
First and foremost, we developed four relevant algorithms using both approaches, explaining each of them in detail for both variants, and showing key differences of the queries. Each algorithm has been tested to answer the central question of where using-key shines, and where it is more of an obstacle than a solution.

For three of the four algorithms - A\*, LCS and Needleman-Wunsch - using-key proves to be a great benefit. The link between these algorithms is that they heavily rely on previous results from more than one iteration ago. One algorithm - Drunken Bishop - proved to suffer from using-key: the overhead of repeated access to the recurring table seems to outweigh the benefit of reduced storage needs. Because Drunken Bishop only ever needs the results from the immediately preceding iteration, a manual carry of old values, as it happens in the classic variants of the other three algorithms, is not needed.
